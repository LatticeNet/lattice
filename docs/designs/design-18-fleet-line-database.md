...
|---|-------|---------|-----------|
| E1 | Extension contract v2.1: `dependencies[]` in the manifest, loader enforcement, plan `checks` section + apply-time revalidation. vpn-core, netguard, wireguard, sub-store manifests updated to declare what they actually need. | — | Enabling an extension with an unmet dep fails with the dep named; an apply with a failed preflight check refuses before touching a node. |
