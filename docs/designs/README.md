...
review-only `/api/dns/plan` that composes the DNS listener into the
`lattice_guard` candidate. CoreDNS apply and Cloudflare publish remain next.
...
| 02 | [Self-hosted DNS](design-02-self-host-dns.md) | One-click private DNS on a chosen node + CF subdomain (gmami-jp1.dns.roobli.org) auto-updated via DDNS + nft-confined | CORE `internal/selfdns`; `DNSDeployment` model/API/dashboard foundation landed iter-033; CoreDNS render + review-only `/api/dns/plan` landed iter-034; apply/publish next; reuses `internal/ddns` (CF) + shared `NFTInputs` |
...
2. **02 Self-host DNS** — continue from iter-034. Next implement the
   rollback-protected CoreDNS/nft apply script, status reconciliation, and then
   reuse DDNS for `/api/dns/publish` / Cloudflare publication.
