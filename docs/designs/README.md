...
| 08 | [Real plugin runners](design-08-real-plugin-runners.md) | Safe path from noop runtime to worker/system/wasm artifact execution | Keep `noop` default; implement worker template runner first, then a constrained system runner for official plugins, then wasm only after an ADR. Marketplace install remains separate from activation, and host-risk plugins keep trusted-signature requirements. |
| 09 | [vpn-core + Sub-Store plugins](design-09-vpn-core-and-substore-plugins.md) | Phased path for turning the Proxy domain into first-party vpn-core/Sub-Store plugins | Engine stays in core until plugin state/encryption/RPC are proven; sing-box JSON automation and Sub-Store companion are incremental, non-breaking slices. |
| 10 | [Plugin dashboard + interfaces](design-10-plugin-dashboard-and-interface-contract.md) | Manifest-declared nav/views and dashboard-to-plugin interface gateway | Plugin UI is declarative data rendered by dashboard primitives; no plugin JS runs in the browser, and gateway calls are scoped and audited. |
