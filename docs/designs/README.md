...
**Status:** *Designed, not yet built.* Each becomes a numbered `iterations/iter-NNN-*.md` when work starts (per `development-workflow.md`: plan → design → build → verify → review → commit).
...
| 04 | [Machine inventory & cost](design-04-machine-inventory-and-cost.md) | Auto-detect CPU/mem/uptime/arch; operator-set cloud vendor/links/cost/renewal + renewal reminders | Low-trust auto `HostFacts` on `model.Node` (never used for authz) + server-only `MachineProfile`; hourly reminder scheduler reusing `internal/notify` |
...
1. **04 Machine inventory (facts) + cost/renewal** — lowest risk, no node mutation, immediately useful (renewal reminders), and produces the `HostFacts`/geo that 05's map needs.
