...
DNSDeployment status reconciliation. Cloudflare publish remains next.
...
| 02 | [Self-hosted DNS](design-02-self-host-dns.md) | One-click private DNS on a chosen node + CF subdomain (gmami-jp1.dns.roobli.org) auto-updated via DDNS + nft-confined | CORE `internal/selfdns`; `DNSDeployment` model/API/dashboard foundation landed iter-033; CoreDNS render + `/api/dns/plan` landed iter-034; rollback-protected apply/status landed iter-035; publish next; reuses `internal/ddns` (CF) + shared `NFTInputs` |
...
2. **02 Self-host DNS** — continue from iter-035. Next implement
   `/api/dns/publish` through `internal/ddns`, then continuous Cloudflare
   publication on node IP changes and dashboard publish/status controls.
