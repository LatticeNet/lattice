...
| **plan → approve → apply** | The deployment path, identical to nft/WireGuard/cftunnel. `POST /api/proxy/nodes/{id}/plan` renders the config and creates a `model.Approval{Plugin:"proxycore"}`; `POST /api/network/approve` (existing handler) with `queue_apply` + optional `plan_sha256` queues the apply task. **Reuse the existing approve endpoint and `applyScriptFor` switch — add one `case "proxycore"`.** |
