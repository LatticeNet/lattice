...
**Batch 2 — landed & verified (commit set 2).** C9 (decoders return a generic "invalid request body"; raw encoding/json strings no longer leak), C15 (approve binds to `plan_sha256`; mismatch → 409, with a regression test). Iter-025 later made this hash mandatory for pending high-risk approvals. Full server/auth/store `-race` green, gofmt clean.
